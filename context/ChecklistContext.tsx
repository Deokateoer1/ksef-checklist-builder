...
interface ChecklistContextType extends ChecklistState {
  ...
  exportToCSV: (data?: ChecklistTask[]) => void;
...
export const ChecklistProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const exportToJSON = (customData?: any) => {
    const data = JSON.stringify(customData || state, null, 2);
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `KSeF_Eksport_${new Date().getTime()}.json`;
    link.click();
    URL.revokeObjectURL(url);
  };

  const exportToCSV = (customTasks?: ChecklistTask[]) => {
    const targetTasks = customTasks || state.tasks;
    if (targetTasks.length === 0) return;

    const headers = ['ID', 'Sekcja', 'Tytuł', 'Priorytet', 'Status', 'Godziny', 'Notatki'];
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `KSeF_Raport_${new Date().getTime()}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  };

  const exportToPDF = (options: ExportOptions) => { /* Logic is in component usually */ };
