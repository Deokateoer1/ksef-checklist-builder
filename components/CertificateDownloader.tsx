...
const CertificateDownloader: React.FC = () => {
  ...
  const isUrgent = diffDays < 30 && diffDays > 0;
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`px-2 py-1 rounded text-[10px] font-black ${isExpired ? 'bg-slate-400 text-white' : isUrgent ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-200 text-slate-600'}`}>
          {isExpired ? 'TERMIN MINĄŁ' : `DEADLINE: 01.02.2026 (${diffDays} dni)`}
        </span>
      </div>
