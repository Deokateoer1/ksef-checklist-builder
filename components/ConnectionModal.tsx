...
const ConnectionModal: React.FC<ConnectionModalProps> = ({ isOpen, onClose }) => {
  ...
  const [robotUrl, setRobotUrl] = useState('http://localhost:8000'); 
    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] w-full max-w-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-10 space-y-10">
          ...
          <section>
             <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Integracja z Robotem (Port 8000)</h4>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Host Robota (HTTP)</label>
                    <input 
                      type="text" 
                      value={robotUrl} 
                  ...
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-2xl">
                    <span className="text-[10px] font-black text-emerald-600 uppercase block mb-1">Status Portu 8000</span>
                    <span className="text-xs font-black text-emerald-900 dark:text-emerald-400">AKTYWNY / DOSTĘPNY</span>
                  </div>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                  ...
                  <ol className="text-[11px] text-slate-500 space-y-2 leading-relaxed">
                    <li>1. Upewnij się, że Twój lokalny serwer FastAPI działa na porcie 8000.</li>
                    <li>2. Wejdź na http://localhost:8000/api/health aby sprawdzić status połączenia.</li>
                    <li>3. Dashboard połączy się automatycznie z Twoją bazą danych.</li>
                  </ol>
                </div>
             </div>
          </section>
