              ...
              <div>
                ...
                <div className="space-y-3">
                  {CRITICAL_DATES.map((d, idx) => {
                    ...
                    return (
                      <div key={idx} className={`p-4 rounded-2xl border ${isPassed ? 'bg-slate-100 opacity-50' : isUrgent ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100'}`}>
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-bold text-slate-700">{d.label}</span>
                          <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${isPassed ? 'bg-slate-400 text-white' : isUrgent ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-900 text-white'}`}>
                            {isPassed ? 'WDROŻONO' : `ZA ${days} DNI`}
                          </span>
                        </div>
                      </div>
                    );
